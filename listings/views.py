...
def progress(request, pk):
    ...
    if request.user == listing.hunter and listing.status != 'Strutting' or request.user == listing.huntee and listing.status != 'Strutting':
        if request.user == listing.hunter and listing.status != 'Payment Issued' or request.user == listing.huntee and listing.status != 'Payment Issued':
            listings = Listings.objects.all()
...
def return_listing(request, pk):
    listing = Listings.objects.get(id=pk)
    if request.user == listing.hunter and listing.status != 'Payment Issued':
        if request.user == listing.hunter and listing.status != 'Complete':
            ...
            content = 'Your listing ' + listing.title + ' has been returned to current listings.'
...
def complete_listing(request, pk):
    listing = Listings.objects.get(id=pk)
    if request.user == listing.hunter and listing.status != 'Strutting':
        if request.user == listing.hunter and listing.status != 'Payment Issued':
...
def issue_payment(request, pk):
    listing = Listings.objects.get(id=pk)
    if request.user == listing.huntee and listing.status == 'Complete':
        if request.method == "POST":
...
def receipt(request, pk):
    listing = Listings.objects.get(id=pk)
    if request.user == listing.huntee and listing.status == 'Payment Issued' or request.user == listing.hunter and listing.status == 'Payment Issued':
        if request.method == "POST":
            return redirect('current_listings')
        else:
            unread_messages = MessagingService.get_unread_messages(request, request.user)
